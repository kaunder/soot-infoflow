...
public class SGPHandlerVerbose implements TaintPropagationHandler{
	...
	//One arg constructor
	public SGPHandlerVerbose(String filename){
	...
	//Iterate over each Abstraction in the outgoing set
		...
		if(outgoing!=null&&!outgoing.isEmpty()){
			...
			String origSC;
			String origIsNull;
			if(temp.getFirstConstr()==3){
				
				if(temp.origNull()){
					origIsNull="ORIG ABSTRACTION = NULL";
				}else{
					origIsNull="ORIG ABSTRACTION NOT NULL";
				}
				
				
				if(temp.getCopyConstrOSC()!=null){
					origSC=temp.getCopyConstrOSC().toString();
				}else{
					origSC="NULL";
				}
			}else{
				origSC="(not copy constr)";
				origIsNull="";
			}
			
			String savedOC;
			if(temp.savedNewSC!=null){
				savedOC=temp.savedNewSC.toString();
			}else{
				savedOC="(Saved New SC was null)";
			}
			
			//If we have null incoming source context, get predecessor and neighbours instead
			String tempPred;
			String tempNeigh;
			if(temp.getSourceContext()==null){
				if(temp.getPredecessor()!=null){
					tempPred=temp.getPredecessor().hashCode()+"\n";
				}else{
					tempPred="(Predecessor was also null)";
				}
				
				if(temp.getNeighbors()!=null&&!temp.getNeighbors().isEmpty()){
					Iterator<Abstraction> it = temp.getNeighbors().iterator();
					tempNeigh="Neighbors: \n";
					while(it.hasNext()){
						tempNeigh=tempNeigh+it.next().hashCode()+"\n";
					}
				}else{
					tempNeigh="(No neighbors)";
				}
			}else{
				tempPred="";
				tempNeigh="";
			...
			//Build output string
			outabs=outabs.concat(srcctxtOUTtxt+srcctxtOUT+"     "+temp.getFirstConstr()+"     "+origSC+"   "+origIsNull+"Predecessor: "+tempPred+"\n"+tempNeigh+"\n"+abshashOUTtxt+abshashOUT+"     "+acspathhashOUT+"     "+uidOUT+"     "+temp.getStmtStr()+"     \n"+tagoutput+"CHILD COUNTER: "+childctr+"\n");
